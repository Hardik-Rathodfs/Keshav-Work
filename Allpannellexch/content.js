...
const urlObserver = new MutationObserver(async () => {
  ...
  if (location.href !== lastUrl) {
    ...
    if (lastUrl.includes("allpanelexch")) {
...
async function handleBetButtonClick(event) {
  ...
  // Get the market-odd-box element
  const marketOddBox = target.closest('.market-odd-box');
  if (marketOddBox) {
    // Get the odds and volume
    const oddsElement = marketOddBox.querySelector('.market-odd');
    const volumeElement = marketOddBox.querySelector('.market-volume');
    
    if (oddsElement && volumeElement) {
      const odds = oddsElement.textContent.trim();
      const volume = volumeElement.textContent.trim();
      
      console.log("Found market odds:", odds, "and volume:", volume);
      
      // If the input amount is negative, use the volume as max value
      if (parseFloat(displayAmount) < 0) {
        betAmount = volume;
        console.log("Using volume as max value:", betAmount);
      }
    }
  }
  ...
  for (let delay of [500, 1000, 2000, 3000]) {
    ...
    // Find the market-nation-name element to get max value as fallback
    const marketNationName = document.querySelector('.market-nation-name');
    if (marketNationName && !betAmount) {
      const text = marketNationName.textContent;
      const maxMatch = text.match(/Max:\s*([\d.]+L?)/);
      ...
      if (maxMatch && maxMatch[1]) {
        ...
        if (parseFloat(displayAmount) < 0) {
          // Keep the original format (e.g. "1L") and remove any decimal points
          betAmount = maxValue.replace(/\.00/g, '');
          console.log("Using max value as fallback:", betAmount);
        } else {
          // For positive values, convert L to actual number if needed
          if (maxValue.includes('L')) {
            maxValue = (parseFloat(maxValue) * 100000).toString();
            // Remove decimal points for positive values too
            maxValue = maxValue.replace(/\.00/g, '');
          }
        }
      }
    }
    ...
    if (stakeInput) {
      ...
      if (inputSuccess) {
        ...
        let placeBetButton = document.querySelector('button.btn.btn-success');
        ...
        if (placeBetButton) {
          ...
          console.log("Place Bet button not found with current selectors");
          
          // Try to find any button that might be the submit button
          const allButtons = document.querySelectorAll('button');
          for (const button of allButtons) {
            const buttonText = button.textContent.trim().toLowerCase();
            if (buttonText.includes('place') || 
                buttonText.includes('confirm') || 
                buttonText.includes('submit') || 
                buttonText.includes('bet')) {
              console.log("Found potential submit button by text:", buttonText);
              ...
              try {
                ...
                // Use the submit() method if it's a form
                const form = button.closest('form');
                if (form) {
                  form.submit();
                }
              } catch (e) {
                console.error("Error clicking potential submit button:", e);
              }
              
              // Track the bet success/failure
              trackBetResult(betAmount, betType);
              return; // Exit function after attempting bet placement
            }
          }
        }
      }
...
async function trackBetResult(amount, betType) {
  ...
  while ((!successToast && !errorToast && !successIndicator && !errorIndicator) && checkCount < 25) {
    ...
    // If we found any indicator, break the loop
  ...
  if (successToast || successIndicator) {
    ...
    // If no indicator found, assume success after a timeout
    console.log("No indicator found, assuming success after timeout");
    
    // Create bet history anyway
    const betDetails = {
      stake: amount,
      odds: oddsValue,
      value: amount,
...
function handleGlobalInput(event) {
  ...
  if (parseFloat(value) < 0) {
    ...
    if (lastClickedElement) {
      ...
      // Only show message if we found a valid max value
      if (contextMaxValue > 0) {
        // showMessage(`Using max bet: ${contextMaxValue}`);
      } else {
        // showMessage("No max bet found yet");
      }
    } else {
