...
function extractMaxBetValue(element, selector) {
  ...
  if (selector === ".fancy-min-max-box") {
    ...
    if (element.textContent.includes("Max:")) {
      ...
      extractedValue = maxSpan
        ? maxSpan.textContent.trim()
        : getMaxValue(element.textContent);
    }
...
function extractMaxBetValueFromContext(element) {
  const contextElement = element.closest('.table-row, .table-header');
  if (!contextElement) return null;

  if (contextElement.classList.contains('table-row')) {
    const tableHeader = contextElement.closest('.table-body').previousElementSibling;
    if (tableHeader && tableHeader.classList.contains('table-header')) {
      const maxValueElement = tableHeader.querySelector('.country-name.box-6.min-max b');
      if (maxValueElement) {
        const text = maxValueElement.textContent.trim();
        const maxMatch = text.match(/Max:([\d.]+k?)/i);
        if (maxMatch) {
          return convertToNumeric(maxMatch[1]);
        }
      }
    }
  } else if (contextElement.classList.contains('table-header')) {
    const maxValueElement = contextElement.querySelector('.country-name.box-6.min-max b');
    if (maxValueElement) {
      const text = maxValueElement.textContent.trim();
      const maxMatch = text.match(/Max:([\d.]+k?)/i);
      if (maxMatch) {
...
function handleGlobalInput(event) {
  ...
  if (value && parseFloat(value) < 0) {
    const maxStakeButton = document.querySelector('.max-stake');
    if (maxStakeButton) {
    ...
    if (maxValue) {
      ...
      if (stakeInput) {
        typeValue(stakeInput, maxValue);
      }
    }
  } else {
    const stakeInput = document.querySelector('input[type="number"][placeholder="00"].form-control.ng-untouched.ng-pristine.ng-valid');
    if (stakeInput) {
      typeValue(stakeInput, value);
    }
  }
}
...
function clickPlaceBetButton() {
  console.log("Attempting to click Place Bet button directly for cbtfair99");
  ...
  let placeBetButton = document.querySelector("button.new-placebet-button");
...
async function handleBetButtonClick(event) {
  ...
  if (parseFloat(displayAmount) < 0) {
    ...
    // Extract max value from the correct context
    const maxValue = extractMaxBetValueFromContext(event.target);
    
    if (maxValue && !isNaN(maxValue) && maxValue > 0) {
      betAmount = maxValue;
      console.log("Using max value for bet:", betAmount);
    } else {
      console.log("No max value found, using original amount");
    }
  }
